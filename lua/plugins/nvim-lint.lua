return {
  ...
  config = function()
    require('lint').linters.snakemake_linter = {
      ...
      args = { "--lint", "--snakefile" }, -- list of arguments. Can contain functions with zero arguments that will be evaluated once the linter is used.
        ...
        for line in lint_output:gmatch("[^\r\n]+") do
          ...
          elseif string.gsub(line,"^%s*(.-)%s*$","%1") == "" then
            ...
            if message_type == "snakefile" then
              _, _, message, linenum, user_data = string.find(current_message,
                "\\*%s+(.-) in line (%d+): (.+)")
              print("s " .. message .. "\n")
              print(user_data .. "\n")
